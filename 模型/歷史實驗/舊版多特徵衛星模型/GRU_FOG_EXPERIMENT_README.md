...
> 以下記錄舊版 SAT / SAT_TIME / SAT_TIME_GEO 實驗及歷史結果。程式目前已改為新版 SAT（16 波段＋6 波段差＋時間＋地理，共 29 特徵），執行方式與輸出見 [新版衛星實驗說明](../../01_衛星模型/README.md)。

這份文件說明舊版 **多測站、60 分鐘後霧偵測**流程與已得到的結果。[專案總說明](../../../README.md) 是目前目錄與資料集背景說明，兩份文件用途不同。
...
| 檔案 | 用途及與本流程的關係 |
|---|---|
| `來源資料/visibility_satellite_2023_clean.parquet` | 2023 年測站、衛星波段、能見度、地理欄位的原始配對資料；本流程唯一資料輸入，Git 已忽略。 |
