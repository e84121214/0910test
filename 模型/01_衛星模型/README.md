...
目前實作三個目標模型中的第一組 `SAT`；`GROUND` 和 `SAT_GROUND` 待地面氣象資料確認後接入。
新的 `SAT` 定義與舊實驗不同，不可將舊 `SAT` 權重當成此模型使用。
...
單層 GRU、32 hidden units、固定種子 42、最多 30 epochs，驗證 loss 連續 5 輪未改善即停止。
...
`ALL` 現階段僅執行已實作的 `SAT`。舊的三組比較結果仍保留於根目錄，不代表新版模型的結果。

## 本次訓練結果

最多 30 輪的訓練於第 11 輪提前停止，最佳權重為第 6 輪；驗證集選出的門檻為 0.73。

| 測試集 | TP | FP | FN | Precision | Recall | CSI |
|---|---:|---:|---:|---:|---:|---:|
| 時間測試 | 87 | 54 | 350 | 61.70% | 19.91% | 17.72% |
| 空間測試 C48 | 6 | 94 | 88 | 6.00% | 6.38% | 3.19% |

時間測試仍漏掉 350/437 筆霧，C48 仍漏掉 88/94 筆霧；模型已可運作，但跨站泛化仍弱。
